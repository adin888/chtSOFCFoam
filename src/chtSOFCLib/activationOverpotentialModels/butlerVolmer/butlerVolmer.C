...
) const
{
    ...
    forAll(stoichiometricNumbers(), fsi)
    {
        if(activeSwitch()[fsi])
        {
            ...
            (
                (pPatch*molFraction[fsi]/constant::thermodynamic::Pstd),
    ...
    (
        -1.*E().value()/(constant::physicoChemical::R.value()*electrodeT)
    );
...
) const
{
    ...
    scalar A = 2*alpha()*eletrochemistryConstant::F.value()/(constant::physicoChemical::R.value()*T);
    scalar B = -2*(1-alpha())*eletrochemistryConstant::F.value()/(constant::physicoChemical::R.value()*T);
